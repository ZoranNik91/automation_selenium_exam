...
def DOM():  #5
    ...
    def TableData7():
        result7 = []
        table = driver.find_element(By.XPATH, '/html/body/div[2]/div/div/div/div/div[2]/table/tbody').get_attribute("textContent").split() #get all data from table and store as array
        for i in table:
            if '7' in i:    #checks if string contains '7'
                result7.append(i)
        result7.sort() # sorted ascendingly, for desc sort set-> sort(reverse=True)
        print(result7)
